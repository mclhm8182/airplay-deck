...
try:  # 游戏模式下 Steam 会带 LD_PRELOAD/LD_LIBRARY_PATH 进来，容器探测要清掉
...
APT_PACKAGES = [
    ...
    # --- mDNS ---
    # ★ 关键：uxplay 用 libavahi-compat-libdnssd，它靠**系统 D-Bus** 找 avahi-daemon。
    #   容器内默认连不上宿主 D-Bus（rootless 的 userns/uid 映射与 EXTERNAL 鉴权打架），
    #   所以要在容器内自建一套 dbus + avahi。这里把两者直接装进镜像，运行时就不用再 apt。
    #   注意 avahi-utils ≠ avahi-daemon：只有 utils 会缺 avahi-daemon 主程序。
    "avahi-utils", "avahi-daemon", "dbus",
]
...
def _container_usable(container: str) -> bool:
    ...
    try:
        ...
        r = subprocess.run(
            ["podman", "exec", *_SANITIZE_ENV, container, "true"],
            capture_output=True, text=True, timeout=15,
        )
        return r.returncode == 0
    except Exception:
...
def _repair_sink(container: str, log_fn: LogFn) -> Tuple[bool, str]:
    ...
    if podman_available():
        try:
            ...
            pass
        cmd = ["podman", "exec", "-u", "0", *_SANITIZE_ENV, container, "bash", "-lc", sh]
    else:
...
def create_and_install(container: str, log_fn: LogFn) -> Tuple[bool, str]:
    ...
    if podman_available():
        ...
        try:
            ...
            pass
        inst_cmd = ["podman", "exec", "-u", "0", *_SANITIZE_ENV, container, "bash", "-lc", install_sh]
    else:
