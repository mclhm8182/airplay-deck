...
- **桌面模式**与 **SteamOS 游戏模式**均已支持。桌面窗口模式会对竖屏流自动最大化投屏窗；游戏模式强制全屏（`-fs`），并包含 X11 鉴权修复与长片投屏保活（systemd-inhibit + 宿主 `xset`）。
...
3. 画面以默认以窗口形式呈现，可随时用任务栏或切回本程序。也可以自主设置为全屏模式。桌面「窗口 / 自动」模式下，竖屏流（如 iPhone 竖拍 Photos 视频）会自动最大化投屏窗口以铺满屏幕（需宿主安装 wmctrl 或 xdotool；游戏模式始终强制全屏 `-fs`）。

**桌面模式与游戏模式**

- **桌面模式：** 在桌面直接打开 AppImage 即可。
- **游戏模式：** 将 AppImage 添加为 Steam「非 Steam 游戏」，从 SteamOS 游戏模式启动。投屏默认全屏。看长视频时请保持「投屏时保持屏幕常亮」开启。
...
| 显示模式 | 窗口 | 窗口：可拖动窗口，竖屏流会自动最大化铺满；全屏：无边框铺满；自动：桌面同窗口（自动最大化），游戏模式强制全屏 |
| 分辨率 | 自动 | 自动时向设备请求 1280×800（Deck 原生）流，画面铺满且不裁切 |
| 音频同步 | 关 | 关：直播式即时出声；开：基于时间戳的严格音画同步（适合看电影） |
| 投屏时保持屏幕常亮 | 开 | uxplay 运行期间阻止系统熄屏；游戏模式另加宿主 `xset` 保活 |
...
gamescope 可能独立于 logind 熄屏。v0.6.11 起在「投屏时保持屏幕常亮」开启时，uxplay 一启动即启用更强的 systemd-inhibit，并在宿主 DISPLAY 上用 xset 周期性保活（停止接收时再强制亮屏一次）。若主机没有 `xset`，会降级为仅 inhibit（仍可能黑屏）——可安装 `xset`，或改用桌面模式。
