...
- 桌面模式稳定可用（推荐；窗口模式会对竖屏流自动最大化投屏窗）；游戏模式有实验性修复（X11 鉴权、强制 `-fs`、长片投屏保活：更强的 systemd-inhibit + 宿主 xset 周期性防熄屏），但**尚未完整支持**。
...
3. 画面以默认以窗口形式呈现，可随时用任务栏或切回本程序。也可以自主设置为全屏模式。桌面「窗口 / 自动」模式下，竖屏流（如 iPhone 竖拍 Photos 视频）会自动最大化投屏窗口以铺满屏幕（需宿主安装 wmctrl 或 xdotool；游戏模式仍强制全屏 `-fs`）。
