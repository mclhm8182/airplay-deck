...
5. 第一次运行前，确保 uxplay 可用（见下）；启动后：
   - **游戏模式**：点「开始接收」窗口自动隐藏，UxPlay 全屏接管；iPhone 控制中心 → 屏幕镜像 → 选设备名。
     按 **Steam 键 → 退出游戏** 停止。
   - **桌面模式**：窗口内点「开始接收」即可，想全屏把「显示模式」设 fullscreen。
...
| 通过 distrobox 运行 | 关 | 开=复用 uxplay-env 容器，免本机安装 uxplay |
...
- **找不到 uxplay**：勾上 App 里的「通过 distrobox 容器运行 uxplay」（复用 uxplay-env）。
...
│   ├── settings.py / uxplay_args.py / session.py / avahi.py / launcher.py
├── resources/icon.svg
├── requirements.txt
└── README.md
```
