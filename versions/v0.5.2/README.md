...
**目标形态：一个自包含的 `AirPlayDeck-<版本号>-x86_64.AppImage` 单文件**（如 `AirPlayDeck-0.5.2-x86_64.AppImage`），放到 Deck 的 `~/Applications`，
...
# 3) 产物 AirPlayDeck-<版本号>-x86_64.AppImage（当前 0.5.2）已生成在 ~/airplay-deck/
exit                         # 退出容器
```
...
1. 把构建产物 `AirPlayDeck-<版本号>-x86_64.AppImage`（当前 `AirPlayDeck-0.5.2-x86_64.AppImage`，文件名带版本号）放到 Deck 的 `~/Applications/`（如不存在就建一个；嫌版本号碍事可重命名为 `AirPlayDeck-x86_64.AppImage`）。
2. 终端执行：`chmod +x ~/Applications/AirPlayDeck-0.5.2-x86_64.AppImage`
...
### uxplay 从哪来？（AppImage 不含 uxplay，但会首次自动安装）
App 默认开启「**通过 distrobox 容器运行 uxplay**」（容器名 `uxplay-env`）。第一次打开 App 时，
若检测到本机还没有这个容器，会**自动弹窗引导你一键安装**：在后台创建 `uxplay-env` 容器，并在其中
`apt` 装好 uxplay + GStreamer 全套插件（首次需联网拉取 Ubuntu 镜像，约 400–800 MB，请耐心等进度条走完）。
...
| v0.5.2 | 对照 UxPlay 1.46 官方 man page 确认其**无 -fs 全屏选项**（与 -vsync/-reset 同被移除），删除 -fs 发射；`display_mode` 不再产生任何 uxplay 参数，三模式统一靠 `-s 1280x800` 的正常窗口铺满屏幕、全画面不裁切；默认 `display_mode` 改为「窗口」。新增容器安装后 `xivmagesink` 渲染后端校验（缺失会触发 renderer->sink 断言崩溃）：自动 `--reinstall` 修复并二次校验，仍失败则明确报错提示修复命令 |
...
| 显示模式 | window | UxPlay 1.46 无任何全屏选项（已对照 man page 核实），故三模式画面表现一致：均靠 `-s 1280x800` 的正常窗口铺满 1280×800 屏幕、全画面不裁切；仅控制 App 是否「接管屏幕」（隐藏主窗口+悬浮条）。默认「窗口」便于多任务；选「自动/全屏」在游戏模式下由 gamescope 把同尺寸窗口呈现为满屏 |
...
- **AppImage 双击无反应 / 启动报错**：SteamOS 镜像**自带 libfuse2**，AppImage 通常能直接跑，无需装任何系统包。
  ...
  `./AirPlayDeck-0.5.2-x86_64.AppImage`（或你实际的版本号文件名）看完整 traceback。若确实报 FUSE 相关错误（极少见），在桌面模式执行
