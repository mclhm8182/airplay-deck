...
def create_and_install(container: str, log_fn: LogFn) -> Tuple[bool, str]:
    ...
    install_sh = (
        ...
        "  echo \"[install] ximagesink 渲染后端缺失，尝试修复…\"\n"
        "  apt-get -o DPkg::Lock::Timeout=600 install -y -f >/dev/null 2>&1 || true\n"
        "  DEBIAN_FRONTEND=noninteractive apt-get -o DPkg::Lock::Timeout=600 install -y --reinstall gstreamer1.0-plugins-base gstreamer1.0-plugins-good >/dev/null 2>&1 || true\n"
        "fi\n"
    ...
    try:
        ...
        if r.returncode != 0:
            return False, ("容器已装好 uxplay，但 ximagesink 渲染后端不可用（UxPlay 启动会崩溃）。"
                          "请在构建环境执行：podman exec -u 0 %s bash -lc "
                          "\"apt-get update && apt-get install -y --reinstall gstreamer1.0-plugins-base gstreamer1.0-plugins-good\"" % container)
    except Exception:
