...
_CTR_MDNS_SETUP = (
    ...
    "_avahi_ok() { dbus-send --system --print-reply --dest=org.freedesktop.Avahi.Server / "
    "org.freedesktop.Avahi.Server.GetVersionString >/dev/null 2>&1; }\n"
    "# 0) 已经能用就什么都不动\n"
    ...
    "  rm -f /run/avahi-daemon/pid   # 容器重启后可能留下过期 pid，会让 avahi 误判「已在运行」\n"
    "  avahi-daemon --no-drop-root --no-chroot --no-rlimits --daemonize "
    ">/tmp/airplay-avahi.log 2>&1 || true\n"
    "  sleep 2\n"
    "fi\n"
    "if _avahi_ok; then _say '容器内 avahi 就绪（mDNS 将直接广播到局域网）'; exit 0; fi\n"
    "_say 'avahi 仍未就绪：'; tail -8 /tmp/airplay-avahi.log 2>&1; exit 11\n"
)
...
def _podman_exec_root(container: str, cmd: str, timeout: float = 300.0):
    ...
    try:
        r = subprocess.run(
            ["podman", "exec", "-u", "0", container, "sh", "-c", cmd],
            capture_output=True, text=True, timeout=timeout,
        )
    except Exception:
        raise
