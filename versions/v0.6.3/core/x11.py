...
def build_podman_cmd(
    ...
    `dbus_address`（可选）：uxplay 靠系统 D-Bus 找 avahi，容器默认地址不通时
    由 `avahi.setup_container_mdns()` 给出的可用地址（宿主的候选路径，或容器内
    自建 bus 的 `unix:path=/run/dbus/system_bus_socket`），用 `-e` 传进去；
    没有它 uxplay 会报 `No DNS-SD Server found` 反复退出。
    """
    mode, path = xauth
