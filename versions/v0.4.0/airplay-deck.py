...
from PyQt6.QtWidgets import (
    ...
    QSystemTrayIcon, QMenu, QMessageBox, QFrame, QScrollArea,
)
...
RUNTIME_KEYS = {
    ...
    "display_mode", "decoder", "reset", "keep_window", "legacy_ports",
...
class FloatingBar(QWidget):
    def __init__(self, on_stop, on_open, text_cb):
        super().__init__(
            None,
        ...
        self.btn_stop.setObjectName("floatStop")
        self.btn_stop.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_stop.clicked.connect(on_stop)
        self.btn_open = QPushButton()
        self.btn_open.setObjectName("floatBtn")
        self.btn_open.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_open.clicked.connect(on_open)
        h.addWidget(self.btn_stop)
        h.addWidget(self.btn_open)
        self.retranslate()

    def retranslate(self):
        self.btn_stop.setText(self._text("float_stop"))
        self.btn_open.setText(self._text("float_open"))
        self.adjustSize()
    ...
    def place_top_center(self):
        ...
        self.move(g.x() + (g.width() - self.width()) // 2, g.y() + 8)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.lang = i18n.normalize(self.settings.get("language", "zh_CN"))
        ...
        self.floating = FloatingBar(self._float_stop, self._float_open, self.T)
        self.floating.hide()
    ...
    def _make_subpage(self, title_text: str):
        ...
        bl = QHBoxLayout(back)
        bl.setContentsMargins(10, 4, 12, 4)
        bl.setSpacing(0)
        arrow = QLabel("‹")
        arrow.setObjectName("backArrow")
        txt = QLabel(self.T("back"))
        txt.setObjectName("backText")
        bl.addWidget(arrow)
        bl.addWidget(txt)
        ...
        h.addWidget(t, 1)
        h.addSpacing(78)  # 抵消左侧返回按钮宽度，让标题视觉居中
        v.addWidget(header)
    ...
    def _build_device_page(self):
        ...
        form.addRow(self.T("name_label"), self.f_name)
        ...
        cur = self._lang_combo.findData(self.lang)
        self._lang_combo.setCurrentIndex(max(0, cur))
        self._lang_combo.currentIndexChanged.connect(self._on_lang_changed)
        form.addRow(self.T("language"), self._lang_combo)
    ...
    def _on_lang_changed(self, _idx: int):
        if self._rebuilding or self._lang_combo is None:
            return
        code = self._lang_combo.currentData() or i18n.DEFAULT_LANG
    ...
    def _build_video_page(self):
        ...
        form.addRow(self.T("fps"), self.f_fps)
        ...
        form.addRow(self.T("video_sink"), self.f_sink)
        ...
        form.addRow(self.T("resolution"), self.f_res)
        ...
        form.addRow(self.T("display_mode"), self.f_mode)
        ...
        form.addRow(self.T("decoder"), self.f_decoder)

        self.f_reset = QComboBox()
        for v in (0, 5, 10, 15, 20, 30, 60):
            self.f_reset.addItem(self.T("none_opt") if v == 0 else str(v), v)
        i = self.f_reset.findData(int(self.settings.get("reset", 15) or 0))
        self.f_reset.setCurrentIndex(max(0, i))
        form.addRow(self.T("reset_label"), self.f_reset)
        rh = QLabel(self.T("reset_hint"))
        ...
        form.addRow(self.T("extra"), self.f_extra)

        tip = QLabel(self.T("tip_video"))
    ...
    def _build_check_page(self):
        ...
        self.f_uxpath.setPlaceholderText(self.T("ux_path_ph"))
        form.addRow(self.T("ux_path"), self.f_uxpath)

        self.f_distrobox = QCheckBox(self.T("use_dbox"))
        ...
        form.addRow(self.T("dbox_name"), self.f_dbox_name)
        ...
        form.addRow(self.T("dbox_method"), self.f_dbox_method)

        tip = QLabel(self.T("tip_dbox"))
        tip.setObjectName("hint")
        tip.setWordWrap(True)
        form.addRow("", tip)
    ...
    def _apply_qss(self) -> None:
        self.setStyleSheet("""
            ...
            QLabel#backArrow { font-size: 30px; color: #e6e7ea; padding: 0 2px 2px 0; }
            QLabel#backText { font-size: 14px; color: #e6e7ea; padding-left: 2px; }
            ...
            QPushButton#backBtn:hover QLabel { color: #4f8cff; }

            QWidget#floatBar { background: #1f2024; border: 1px solid #3a3c41; border-radius: 14px; }
            QPushButton#floatStop { background: #c0392b; color: #fff; border: none;
                padding: 8px 16px; font-size: 13px; font-weight: 600; border-radius: 8px; }
            QPushButton#floatStop:hover { background: #d94a3a; }
            QPushButton#floatBtn { background: #2f3136; color: #e6e7ea;
                border: 1px solid #3a3c41; padding: 8px 16px; font-size: 13px; border-radius: 8px; }
            QPushButton#floatBtn:hover { background: #3a3c41; }
    ...
    def _load_ui_from_settings(self) -> None:
        ...
        self.f_decoder.setCurrentIndex(max(0, i))
        i = self.f_reset.findData(int(s.get("reset", 15) or 0))
        self.f_reset.setCurrentIndex(max(0, i))
    ...
    def _start(self):
        ...
        self.launcher.start()
        self.toggle.setChecked(True)
        # 游戏模式：隐藏主窗口让 uxplay 全屏接管，但保留顶部悬浮控制条，
        # 否则一旦 uxplay 出问题就再也回不到 App（黑屏且无出口）。
        if session.is_gamemode():
            self.hide()
            self.floating.show_bar()
    ...
    def _on_status(self, status: str):
        ...
        # 游戏模式出错 / 断开：把主窗口放回来，避免用户被困在黑屏里
        if session.is_gamemode() and status in ("error", "idle"):
            if self.launcher is not None or status == "error":
                self.floating.show_bar()
    ...
    def _collect(self) -> dict:
        ...
        s["reset"] = int(self.f_reset.currentData() or 0)
        ...
        if self._lang_combo is not None:
            s["language"] = i18n.normalize(self._lang_combo.currentData() or self.lang)
        return s
    ...
    def _check_uxplay(self):
        ...
        if use_dbox:
            ...
            where = f"distrobox 容器 {container}"
        else:
            cmd = [binpath, "-v"]
            where = "系统 PATH"
        try:
    ...
    def _test_container(self):
        if not self.settings.get("use_distrobox"):
            ...
            return
        container = (self.settings.get("distrobox_container") or "uxplay-env").strip()
        self._log("info", f"测试容器 {container}（桌面模式会弹图形授权）…")
        try:
            r = subprocess.run(
    ...
    def closeEvent(self, event):
        if getattr(self, "tray", None) and self.tray.isVisible():
            self.hide()
            if session.is_gamemode() and self.launcher is not None:
                self.floating.show_bar()
            event.ignore()
        else:
            self._quit()
