...
class Launcher:
    ...
    def _mark_fatal(self) -> None:
        ...
        self._log("error",
                  ...
                  "  2) **ximagesink 渲染后端损坏**：若日志出现 `Assertion 'renderer->sink' failed`，"
                  "是 GStreamer 注册表缓存过期所致。在构建环境执行：\n"
                  "     podman exec -u 0 uxplay-env bash -lc \"apt-get update && "
                  "apt-get install -y --reinstall gstreamer1.0-plugins-base gstreamer1.0-plugins-good "
    ...
    def _pump(self, proc: subprocess.Popen) -> None:
        ...
        FATAL_HINTS = (
            ...
            "renderer->sink",        # UxPlay 1.46 video_renderer_init 断言：ximagesink 后端不可用
