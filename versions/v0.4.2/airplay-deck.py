...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._fs = False  # 本次接收是否以全屏接管（游戏模式必然全屏；桌面模式看显示模式）
    ...
    def _build_video_page(self):
        ...
        dm_hint = QLabel(self.T("dm_hint"))
        dm_hint.setObjectName("hint")
        dm_hint.setWordWrap(True)
        form.addRow(self._form_label(""), dm_hint)
        ...
        form.addRow(self._form_label(self.T("reset_label")), self.f_reset)
        rh = QLabel(self.T("reset_hint"))
        rh.setObjectName("hint")
        rh.setWordWrap(True)
        form.addRow(self._form_label(""), rh)
        ...
        form.addRow(self._form_label(""), tip)
    ...
    def _will_fullscreen(self) -> bool:
        """本次接收是否以全屏方式接管屏幕。

        游戏模式必然全屏（gamescope 只呈现全屏 X11 窗口，窗口化不可见）；
        桌面模式则由「显示模式」决定：选「全屏」(默认) 即全屏，选「窗口」保留
        普通可拖动窗口，选「自动」则游戏全屏 / 桌面窗口。
        """
        if session.is_gamemode():
            return True
        return (self.settings.get("display_mode") or "fullscreen") == "fullscreen"
    ...
    def _start(self):
        ...
        self.launcher.start()
        self.toggle.setChecked(True)
        # 全屏接管：游戏模式必然全屏；桌面模式若「显示模式」选了全屏（默认），
        # 同样隐藏主窗口并保留顶部悬浮控制条，否则一旦 uxplay 出问题就回不到 App。
        self._fs = self._will_fullscreen()
        if self._fs:
            self.hide()
    ...
    def _on_status(self, status: str):
        ...
        # 全屏模式出错 / 断开：把悬浮控制条放回来，避免用户被困在黑屏里
        if self._fs and status in ("error", "idle"):
    ...
    def closeEvent(self, event):
        if getattr(self, "tray", None) and self.tray.isVisible():
            self.hide()
            if self._fs and self.launcher is not None:
                self.floating.show_bar()
            event.ignore()
        else:
            self._quit()
