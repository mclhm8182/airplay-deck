...
**目标形态：一个自包含的 `AirPlayDeck-<版本号>-x86_64.AppImage` 单文件**（如 `AirPlayDeck-0.6.2-x86_64.AppImage`），放到 Deck 的 `~/Applications`，
...
# 3) 产物 AirPlayDeck-<版本号>-x86_64.AppImage（当前 0.6.2）已生成在 ~/airplay-deck/
exit                         # 退出容器
```
...
2. 把本目录所有文件（含 `.github/workflows/build-appimage.yml`）推上去：
   ...
   cd airplay-deck
   git init -b main
   git add -A && git commit -m "airplay-deck v0.6.2"
...
4. **推荐：直接去 Releases 页下载**（`release` job 会自动把 AppImage 挂上去）：
   ...
   - 每次推送会创建/更新标签形如 `v0.6.2-rc` 的 **prerelease**，资产就是
     `AirPlayDeck-0.6.2-x86_64.AppImage`（约 98 MB），点一下即下，不用解压 zip。
   - 直链形如：`https://github.com/<你的账号>/airplay-deck/releases/download/v0.6.2-rc/AirPlayDeck-0.6.2-x86_64.AppImage`
...
1. 把构建产物 `AirPlayDeck-<版本号>-x86_64.AppImage`（当前 `AirPlayDeck-0.6.2-x86_64.AppImage`，文件名带版本号）放到 Deck 的 `~/Applications/`（如不存在就建一个；嫌版本号碍事可重命名为 `AirPlayDeck-x86_64.AppImage`）。
2. 终端执行：`chmod +x ~/Applications/AirPlayDeck-0.6.2-x86_64.AppImage`
...
- **AppImage 双击无反应 / 启动报错**：SteamOS 镜像**自带 libfuse2**，AppImage 通常能直接跑，无需装任何系统包。
  ...
  `./AirPlayDeck-0.6.2-x86_64.AppImage`（或你实际的版本号文件名）看完整 traceback。若确实报 FUSE 相关错误（极少见），在桌面模式执行
...
- **日志刷 `*** ERROR: No DNS-SD Server found (DNSServiceRegister call returned kDNSServiceErr_Unknown)`，uxplay 存活 1 秒就退出并反复重启，手机/电脑搜不到设备**：
  这是**容器内连不上宿主机的 mDNS（avahi）**。uxplay 通过 `libavahi-compat-libdnssd` 走**系统 D-Bus** 找 avahi-daemon，
  容器里没有宿主的系统 D-Bus socket 时就会这样。先跑这条命令看实况：
  ```bash
  podman exec uxplay-env sh -c 'ls -l /run/dbus /run/host/run/dbus 2>&1; which dbus-send avahi-browse; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/host/run/dbus/system_bus_socket dbus-send --system --print-reply --dest=org.freedesktop.Avahi.Server / org.freedesktop.Avahi.Server.GetVersionString'
  ```
  若第二种地址能返回 avahi 版本号，**v0.6.2 起 App 会自动改用它**（日志会写「已改用 unix:path=… 连宿主机 mDNS 服务」），
  不必手工设置；「导出日志」也已自动附带这段诊断。宿主侧可再确认 avahi 在跑、且 `avahi-daemon.conf` 里没有
  `disable-publishing=yes`：`systemctl status avahi-daemon`、`avahi-browse -a -t`。
