...
class MainWindow(QMainWindow):
    ...
    def _will_takeover(self) -> bool:
        """本次接收是否以「接管屏幕」方式呈现（隐藏主窗口 + 显示悬浮控制条）。

        只有**游戏模式**需要接管：gamescope 只呈现 uxplay 的全屏 X11 窗口，用户看不到
        任务栏、也点不回本 App，必须留一个置顶悬浮条作为返回入口。

        桌面模式一律不接管：uxplay 自己的窗口（auto 铺满 1280×800 / window 小窗 /
        fullscreen 无边框）就是画面本体，桌面有任务栏与 Alt-Tab 可随时切回，
        再加一个置顶悬浮条纯属多余干扰（用户 2026-09-11 明确要求去掉）。
        """
        return session.is_gamemode()
    ...
    def _start(self):
        ...
        self.launcher.start()
        self.toggle.setChecked(True)
        # 仅游戏模式「接管屏幕」：隐藏主窗口 + 顶部悬浮控制条（否则 gamescope 里回不到 App）。
        # 桌面模式保留主窗口、不出悬浮条 —— uxplay 窗口会盖在它上面显示画面，
        # 用户随时可用任务栏 / Alt-Tab 切回，不需要那条置顶控制条。
        self._fs = self._will_takeover()
        if self._fs:
            self.hide()
    ...
    def _on_status(self, status: str):
        ...
        # 游戏模式接管屏幕后出错 / 断开：把悬浮控制条放回来，避免用户被困在黑屏里
        # （桌面模式不出悬浮条，_fs 恒为 False，不会走到这里）
