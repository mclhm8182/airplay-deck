...
from . import avahi, session, uxplay_args, x11
    ...
    def _run(self) -> None:
        ...
        if self.settings.get("use_distrobox"):
            ...
            if is_gm and method != "podman":
                ...
                method = "podman"
            # ★ uxplay 用 libavahi-compat-libdnssd，它靠**系统 D-Bus** 连宿主 avahi。
            #   容器里若没有这个 socket，uxplay 会报 No DNS-SD Server found 并反复退出
            #   （客户端永远搜不到设备）。这里先探一次，必要时换个 D-Bus 地址。
            try:
                subprocess.run(["podman", "start", container],
                               capture_output=True, timeout=30.0)
            except Exception:
                pass  # 已在运行会报错，忽略
            dbus_addr = avahi.container_dbus_address(container, self._log)
            if method == "podman":
                # 直接 podman start + exec：rootless 容器无需任何授权，
                # 可绕开游戏模式下 distrobox enter 弹不出 polkit 对话框导致 uxplay 起不来的问题。
                display = os.environ.get("DISPLAY", ":0")
                # X11 鉴权：枚举候选并在容器内用 ximagesink 实测，选真能连上的那个
                ctr_xauth = x11.probe_container_xauth(container, display, self._log)
                ...
                if dbus_addr:
                    # 用 env 传变量，避免再套一层 sh -c 改变进程树
                    cmd += ["env", f"DBUS_SYSTEM_BUS_ADDRESS={dbus_addr}"]
                cmd += [binpath, *args]
