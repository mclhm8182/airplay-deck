...
**目标形态：一个自包含的 `AirPlayDeck-<版本号>-x86_64.AppImage` 单文件**（如 `AirPlayDeck-0.6.5-x86_64.AppImage`），放到 Deck 的 `~/Applications`，
...
# 3) 产物 AirPlayDeck-<版本号>-x86_64.AppImage（当前 0.6.5）已生成在 ~/airplay-deck/
exit                         # 退出容器
```
...
2. 把本目录所有文件（含 `.github/workflows/build-appimage.yml`）推上去：
   ...
   cd airplay-deck
   git init -b main
   git add -A && git commit -m "airplay-deck v0.6.5"
...
4. **推荐：直接去 Releases 页下载**（`release` job 会自动把 AppImage 挂上去）：
   ...
   - 每次推送会创建/更新标签形如 `v0.6.5-rc` 的 **prerelease**，资产就是
     `AirPlayDeck-0.6.5-x86_64.AppImage`（约 100 MB），点一下即下，不用解压 zip。
   - 直链形如：`https://github.com/<你的账号>/airplay-deck/releases/download/v0.6.5-rc/AirPlayDeck-0.6.5-x86_64.AppImage`
...
1. 把构建产物 `AirPlayDeck-<版本号>-x86_64.AppImage`（当前 `AirPlayDeck-0.6.5-x86_64.AppImage`，文件名带版本号）放到 Deck 的 `~/Applications/`（如不存在就建一个；嫌版本号碍事可重命名为 `AirPlayDeck-x86_64.AppImage`）。
2. 终端执行：`chmod +x ~/Applications/AirPlayDeck-0.6.5-x86_64.AppImage`
...
| 版本 | 主要变化 |
|------|----------|
| v0.6.5 | 修 v0.6.4 实机暴露的两处**误判**，外加把「卡顿」的排查方向指对。**(1) mDNS「未就绪」是误报**：avahi 注册的**总线名**是 `org.freedesktop.Avahi`，而 `org.freedesktop.Avahi.Server` 是它的**接口名**——过去一直用 `dbus-send --dest=org.freedesktop.Avahi.Server … GetVersionString` 探测，永远得到 `ServiceUnknown`，于是每次都误判「容器内 avahi 未就绪」+ 每次都白跑一遍自建 dbus/avahi（还连带 kill/重启 avahi），而实际上 mDNS 一直是好的（v0.6.4 日志里 iPad 明明连上并投屏了）。现改为看 `ListNames` 里有没有 `org.freedesktop.Avahi`，与 avahi 版本无关；既不再误报，也省掉每次启动那几秒无用功。**(2) 游戏模式连不上 X 的真因不是显示号，而是 cookie 候选被垃圾文件占满**：游戏模式 `/run/user/<uid>` 下有一堆 `gamescope*` 文件，`find_host_xauth_candidates()` 按 glob 排出来的头两个正好是 `gamescope-environment` / `gamescope-stats`（**文本/统计文件，不是 X cookie**），而每个显示号只实测 `_MAX_PER_DISPLAY=5` 个组合 —— 名额被它们占满，真正的 `gamescope.XXXXXX`（Xwayland 的 `-auth` 文件）**一次都没被试过**，所以 `:1` 和 `:0` 全军覆没（v0.6.4 里「改用 :0」的结论是错的，见下方排错段）。现在新增 `find_x_server_auth_map()`：扫宿主机 `/proc/*/cmdline`，把 Xorg/Xwayland 命令行里 `-auth <文件>` 的**权威答案**按显示号取出来，该文件排在最前先试；同时把 `environment`/`stats`/`log` 这类明显不是 cookie 的文件排到最后，`_MAX_PER_DISPLAY` 提到 8。**(3) 卡顿排查**：日志出现 `raop_rtp resend failed` / `since last client feedback request` 时给一次性提示（这是丢包，属网络侧），并在选了 `xvimagesink` 时提示改回 `ximagesink` |
...
- **AppImage 双击无反应 / 启动报错**：SteamOS 镜像**自带 libfuse2**，AppImage 通常能直接跑，无需装任何系统包。
  ...
  `./AirPlayDeck-0.6.5-x86_64.AppImage`（或你实际的版本号文件名）看完整 traceback。若确实报 FUSE 相关错误（极少见），在桌面模式执行
