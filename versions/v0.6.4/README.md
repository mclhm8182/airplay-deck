...
**目标形态：一个自包含的 `AirPlayDeck-<版本号>-x86_64.AppImage` 单文件**（如 `AirPlayDeck-0.6.4-x86_64.AppImage`），放到 Deck 的 `~/Applications`，
...
# 3) 产物 AirPlayDeck-<版本号>-x86_64.AppImage（当前 0.6.4）已生成在 ~/airplay-deck/
exit                         # 退出容器
```
...
2. 把本目录所有文件（含 `.github/workflows/build-appimage.yml`）推上去：
   ...
   cd airplay-deck
   git init -b main
   git add -A && git commit -m "airplay-deck v0.6.4"
...
4. **推荐：直接去 Releases 页下载**（`release` job 会自动把 AppImage 挂上去）：
   ...
   - 每次推送会创建/更新标签形如 `v0.6.4-rc` 的 **prerelease**，资产就是
     `AirPlayDeck-0.6.4-x86_64.AppImage`（约 100 MB），点一下即下，不用解压 zip。
   - 直链形如：`https://github.com/<你的账号>/airplay-deck/releases/download/v0.6.4-rc/AirPlayDeck-0.6.4-x86_64.AppImage`
...
1. 把构建产物 `AirPlayDeck-<版本号>-x86_64.AppImage`（当前 `AirPlayDeck-0.6.4-x86_64.AppImage`，文件名带版本号）放到 Deck 的 `~/Applications/`（如不存在就建一个；嫌版本号碍事可重命名为 `AirPlayDeck-x86_64.AppImage`）。
2. 终端执行：`chmod +x ~/Applications/AirPlayDeck-0.6.4-x86_64.AppImage`
...
- **AppImage 双击无反应 / 启动报错**：SteamOS 镜像**自带 libfuse2**，AppImage 通常能直接跑，无需装任何系统包。
  ...
  `./AirPlayDeck-0.6.4-x86_64.AppImage`（或你实际的版本号文件名）看完整 traceback。若确实报 FUSE 相关错误（极少见），在桌面模式执行
...
- **游戏模式搜得到但一投就退（`Failed to initialize GStreamer video renderer` / `Authorization required, but no authorization protocol specified`）**：
  App 被 Steam 拉起时继承的是 gamescope 的 `DISPLAY=:1`，但容器里 `:1` 连不上 X（鉴权失败），真实可用的是 `:0`。
  **v0.6.4 起** `probe_container_display()` 会自动逐个实测 DISPLAY 候选并切到能用的那个，日志会写 `继承的 DISPLAY=:1 在容器里不可用，已改用 :0`。
  同时所有容器命令都加了 `-e LD_PRELOAD= -e LD_LIBRARY_PATH=`，避免 Steam 的 32 位 `gameoverlayrenderer.so` 污染容器进程
...
- **日志刷 `*** ERROR: No DNS-SD Server found (DNSServiceRegister call returned kDNSServiceErr_Unknown)`，uxplay 存活 1 秒就退出并反复重启，手机/电脑搜不到设备**：
  ...
  **v0.6.3 起 App 会自动在容器内自建一套 dbus + avahi**，正常情况下你什么都不用做——日志会写
  ...
  若仍失败，先跑这条命令看实况：
  ```bash
  podman exec uxplay-env sh -c 'ls -l /run/dbus /run/host/run/dbus /run/avahi-daemon 2>&1; which dbus-send dbus-daemon avahi-daemon; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/dbus/system_bus_socket dbus-send --system --print-reply --dest=org.freedesktop.Avahi.Server / org.freedesktop.Avahi.Server.GetVersionString'
  ```
  也可以手工在容器内起一套（与 App 做的是同一件事，免密码）：
  ```bash
  podman exec -u 0 uxplay-env sh -c 'apt-get install -y dbus avahi-daemon; mkdir -p /run/dbus /run/avahi-daemon; dbus-uuidgen --ensure; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/dbus/system_bus_socket dbus-daemon --system --fork --nopidfile; avahi-daemon --no-drop-root --no-chroot --no-rlimits --daemonize; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/dbus/system_bus_socket dbus-send --system --print-reply --dest=org.freedesktop.Avahi.Server / org.freedesktop.Avahi.Server.GetVersionString'
  ```
