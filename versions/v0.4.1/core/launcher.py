...
class Launcher:
    ...
    def _run(self) -> None:
        ...
        if self.settings.get("use_distrobox"):
            ...
            if method == "podman":
                ...
                # X11 鉴权：宿主机的 cookie 路径在容器里通常不存在/不可读，会报
                #   "Authorization required, but no authorization protocol specified"
                #   "GStreamer error: Could not initialise X output"
                # 这里枚举候选方案并在容器内实测，选中真能连上 X 的那个。
                ctr_xauth = x11.probe_container_xauth(container, display, self._log)
                cmd = ["podman", "exec", "-e", f"DISPLAY={display}"]
                if ctr_xauth:
                    cmd += ["-e", f"XAUTHORITY={ctr_xauth}"]
                cmd += [
    ...
    def _mark_fatal(self) -> None:
        ...
        self._log("error",
                  "最常见原因是**容器里连不上 X**：宿主机的 X cookie 在容器内不存在或不可读。"
                  "请点「导出日志」看上面 X11 探测那几行；"
                  "或在「检查与日志」里把容器名/进入方式调整后重试"
                  "（也可临时取消勾选「通过容器运行 uxplay」改用本机 uxplay 验证）。")
        self._status("error")
