...
class MainWindow(QMainWindow):
    ...
    def _build_about_page(self):
        ...
        cv.addWidget(text)
        # GitHub 仓库跳转：用 QPushButton（图标 + 链接文字），点击 100% 可靠打开浏览器。
        ...
        _gh_btn.setStyleSheet(
            "QPushButton#ghLink { background: transparent; border: none; color: #58a6ff;"
            " font-size: 12px; padding: 2px 4px; text-align: left; spacing: 6px; }"
            "QPushButton#ghLink:hover { color: #79c0ff; text-decoration: underline; }"
        )

        def _open_gh() -> None:
            QDesktopServices.openUrl(QUrl(_gh_url))

        def _copy_gh() -> None:
            try:
                QApplication.clipboard().setText(_gh_url)
            except Exception:
                pass

        _gh_btn.clicked.connect(_open_gh)
        _gh_btn.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        _gh_btn.customContextMenuRequested.connect(lambda _pos: _copy_gh())
