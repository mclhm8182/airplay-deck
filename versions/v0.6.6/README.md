...
**目标形态：一个自包含的 `AirPlayDeck-<版本号>-x86_64.AppImage` 单文件**（如 `AirPlayDeck-0.6.6-x86_64.AppImage`），放到 Deck 的 `~/Applications`，
...
# 3) 产物 AirPlayDeck-<版本号>-x86_64.AppImage（当前 0.6.6）已生成在 ~/airplay-deck/
exit                         # 退出容器
```
...
2. 把本目录所有文件（含 `.github/workflows/build-appimage.yml`）推上去：
   ...
   cd airplay-deck
   git init -b main
   git add -A && git commit -m "airplay-deck v0.6.6"
...
4. **推荐：直接去 Releases 页下载**（`release` job 会自动把 AppImage 挂上去）：
   ...
   - 每次推送会创建/更新标签形如 `v0.6.6-rc` 的 **prerelease**，资产就是
     `AirPlayDeck-0.6.6-x86_64.AppImage`（约 100 MB），点一下即下，不用解压 zip。
   - 直链形如：`https://github.com/<你的账号>/airplay-deck/releases/download/v0.6.6-rc/AirPlayDeck-0.6.6-x86_64.AppImage`
...
1. 把构建产物 `AirPlayDeck-<版本号>-x86_64.AppImage`（当前 `AirPlayDeck-0.6.6-x86_64.AppImage`，文件名带版本号）放到 Deck 的 `~/Applications/`（如不存在就建一个；嫌版本号碍事可重命名为 `AirPlayDeck-x86_64.AppImage`）。
2. 终端执行：`chmod +x ~/Applications/AirPlayDeck-0.6.6-x86_64.AppImage`
3. Steam → 库 → 添加非 Steam 游戏 → 浏览，选中这个 `.AppImage`。
4. 右键该条目 → 属性 → 名称改成「AirPlay Deck」。
5. 第一次运行前，确保 uxplay 可用（见下）；启动后：
   - **游戏模式**：点「开始接收」窗口自动隐藏，UxPlay 全屏接管；顶部会出现一条**悬浮控制条**（停止接收 / 回到设置）——这是游戏里唯一的返回入口。iPhone 控制中心 → 屏幕镜像 → 选设备名。
     按 **Steam 键 → 退出游戏** 停止。
   - **桌面模式**：**不隐藏本 App 窗口、不出现悬浮控制条**。UxPlay 以普通窗口呈现画面——「自动」铺满 1280×800 屏幕（全画面、不裁切、非无边框全屏），「窗口」是可拖动小窗，「全屏」为无边框 `-fs`；随时用任务栏 / `Alt+Tab` 切回本 App。
...
| 版本 | 主要变化 |
|------|----------|
| v0.6.6 | 修 v0.6.5 实机暴露的两个体验问题 + 把游戏模式 X 鉴权的根因彻底修掉。**(1) 刷短视频时声音晚几秒才跟上的卡顿**：UxPlay 1.73.7 默认 `-vsync`（基于时间戳的 A/V 同步），每个新视频开头它会先对齐视频再放音，于是「下一个视频声音晚几秒」。`core/uxplay_args.py` 新增 `audio_sync` 开关（设置项，默认 `off`），选 `off` 时发射 `-vsync no`——改走「直播式」即时出声、不丢帧、不再为对齐视频延迟音频，刷小红书/玩游戏/投屏浏览的卡顿感消失；想看电影追求严丝合缝可在「视频与渲染」里把「音频同步」勾上（恢复基于时间戳同步）。**(2) 游戏模式仍连不上 X 的真因不是显示号、也不是 cookie 被垃圾文件占满，而是 v0.6.5 的 `_MAX_PER_DISPLAY=8` 上限把本该试的 cookie 默默截断了**：游戏模式下 Xwayland 的 cmdline 在 `/proc` 里读不到（`find_x_server_auth_map()` 返回空），于是退化到 glob 候选——而 v0.6.5 只测了前 4 个 `gamescope.*`、后 2 个（含真正的 Xwayland `-auth`）被预算吃掉、`rc=255` 全部失败。`core/x11.py` 重写：`_parse_xauthority()` 按二进制结构解析 Xauthority（区分真 cookie 与 `gamescope-environment`/`gamescope-stats` 这类伪文件）、真实 cookie 按 mtime 倒序优先当前会话、新增第二权威来源——扫 `/proc/<pid>/environ` 取 `DISPLAY`+`XAUTHORITY`、去掉每显示号上限改为**枚举全部候选**并在失败时抓取 stderr 里的 `reason`、新增宿主机 `libX11` 的 `XOpenDisplay` 实测诊断（`host_can_open_display`）、`build_podman_cmd` 注入 `XDG_RUNTIME_DIR`。现在游戏模式能正确拿到 Xwayland 的 `-auth` 连上 X、发现设备并投屏。**(3) iPhone 相册视频被缩小、iPad 正常**：这是 `-s 1280x800` 高度锚定的协商产物（iPhone 竖屏比例更瘦，客户端发来的流在 1280×800 窗口里居中留边显得小；iPad 比例接近所以正常）。属低优先级，暂保持现状；如介意可在「额外参数」里加 `-vs "ximagesink force-aspect-ratio=true"` 试（ximagesink 对该属性的支持不确定） |
| v0.6.4 | 修 v0.6.3 实机暴露的三个问题。**(1) 桌面/游戏模式都没声音**：容器内 `autoaudiosink` 找不到可用输出（ALSA 无卡、pulse 连不上、JACK 未运行），而 SteamOS 的声音是 PipeWire、在 `/run/user/1000/pulse/native` 暴露 PulseAudio 兼容 socket（容器共享挂载，能直接看到 `/run/host/run/user/1000/pulse/native`）。新增 `core/audio.py`：开始接收前在容器里逐个实测候选 socket × cookie（跑一段 `volume=0` 的 `gst-launch audiotestsrc ! pulsesink` 静音测试），第一个通的就用，把 `PULSE_SERVER=unix:<sock>`（必要时加 `PULSE_COOKIE`）通过 `podman exec -e` / `distrobox enter -- env` 透传给 uxplay；缺 `pulsesink` 会自动 `apt-get install gstreamer1.0-pulseaudio` + 清注册表缓存。探测失败也不阻断投屏（照常出画面，日志写明）。**(2) 游戏模式搜不到设备 / `Failed to initialize GStreamer video renderer`**：App 被 Steam 拉起时继承了 gamescope 的 `DISPLAY=:1`，但容器里 `:1` 连不上（各种 `XAUTHORITY` 组合都报 `Authorization required, but no authorization protocol specified`），真实可用的是 `:0`。`core/x11.py` 的 `probe_container_display()` 现在会枚举 DISPLAY 候选（继承值 → 环境值 → `:0` → `:1`）逐个用 `ximagesink` 探针实测，选第一个真正连得上的（带总预算 100s 上限），日志会写「继承的 DISPLAY=:1 在容器里不可用，已改用 :0」。**(3) 游戏模式不再要悬浮控制条**：按使用反馈彻底删掉 `FloatingBar`，`_will_takeover()` 恒返回 `False`（不再隐藏主窗口、不再置顶控制条），投屏是直接全屏出画面；`_on_status` 记录「是否刚连过」`_was_connected`，一旦从 `connected` 退回 `waiting/idle/error` 就自动把主窗口拉回来（日志「投屏已结束，已切回主界面」）。顺带：**所有容器 `podman exec` 都注入 `-e LD_PRELOAD= -e LD_LIBRARY_PATH=`**——Steam 会把 32 位 `gameoverlayrenderer.so` 塞进 `LD_PRELOAD` 并一路继承进容器，污染 avahi/dbus/uxplay（`ld.so: wrong ELF class: ELFCLASS32`），现在显式置空；容器内 avahi 改为 `setsid … --debug` 写日志 + 等待轮询，失败时 tail 出真实原因（`--daemonize` 会把 stdio 关掉，之前只看得到 3 行 ld.so 报错） |
...
| v0.6.1 | **引擎大升级：从 apt 的 UxPlay 1.46 换成自编译 UxPlay 1.73.7**（跨 4 年）。原因：1.46 选项集老旧且含未修复漏洞。`core/container.py` 不再 apt 装 uxplay，改为装 `build-essential/cmake/libssl-dev/libplist-dev/libavahi-compat-libdnssd-dev/libgstreamer*-dev/libx11-dev` 后，从 GitHub 拉 v1.73.7 源码 `cmake && make && make install` 到 `/usr/local/bin`（依次尝试 3 个官方地址；GitHub 不可达时可手动把源码包放 `~/uxplay-1.73.7.tar.gz` 走离线分支）；`uxplay_installed()` 同时探测 `/usr/local/bin`。收益：**恢复 `-fs` 全屏**（`core/uxplay_args.py` 重新按 display_mode 发射：window 不发 / fullscreen 发 / auto 仅游戏模式发），并修掉 `raop_handlers.h` 栈溢出+空指针（GHSA-479c-ww7g-wgp8）、iOS 27 适配、多处 segfault |
| v0.6.3 | 根治「手机/电脑搜不到设备、日志刷 `No DNS-SD Server found (kDNSServiceErr_Unknown)`」。v0.6.2 的「换 D-Bus 路径」被实机日志证伪：rootless 容器里 `/run/dbus` **根本不存在**，唯一存在的 `/run/host/run/dbus/system_bus_socket` 连上去是 `Did not receive a reply`——那是 rootless 的 user-namespace / uid 映射与 D-Bus EXTERNAL 鉴权打架，换 socket 路径治不好。新方案：**不再碰宿主的 D-Bus**，改为在容器内**自建一套 dbus-daemon + avahi-daemon**（容器与宿主共享网络命名空间，一样广播到局域网，客户端完全无感）。开始接收前 `avahi.setup_container_mdns()` 三步走：容器默认地址 → 宿主候选路径 → 容器内自建；自建脚本会按需补装 `dbus`/`avahi-daemon`（注意旧容器里 `avahi-utils ≠ avahi-daemon`）、`dbus-uuidgen --ensure`、清掉 distrobox 放在 `/run/avahi-daemon/socket` 的宿主符号链接、依次拉起 `dbus-daemon --system` 与 `avahi-daemon --no-drop-root --no-chroot --no-rlimits`，每步都写日志并用 `dbus-send … GetVersionString` 验证。`APT_PACKAGES` 也直接补进 `dbus`/`avahi-daemon`，新容器一次装好 |
| v0.6.2 | 修复「搜得到/连得上却没有设备可搜」的另一类根因：**容器里连不上宿主机的 mDNS（avahi）**。uxplay 用的是 `libavahi-compat-libdnssd`，它**靠系统 D-Bus**（不是 `/run/avahi-daemon/socket`）找 avahi-daemon；容器里没有宿主的系统 D-Bus socket 时，uxplay 会打印 `*** ERROR: No DNS-SD Server found (DNSServiceRegister call returned kDNSServiceErr_Unknown)`、退出码 0、存活 1 秒，然后被无限重启——**手机/电脑永远搜不到设备**。现在开始接收前会先在容器内实测能否用 D-Bus 问到 avahi：默认地址不通就自动改用 `/run/host/run/dbus/system_bus_socket`（distrobox 把宿主根挂在 `/run/host`）并显式导出 `DBUS_SYSTEM_BUS_ADDRESS`（`podman exec -e` / `distrobox enter -- env …`），日志会写明用了哪个地址；探测不到时直接给出可粘贴的排查命令。「一键检查运行环境」也新增了容器内 mDNS 连通性检查，「导出日志」自动附带容器内 mDNS/D-Bus 实况（socket 是否存在、`dbus-send`/`avahi-browse` 是否可用、两种地址各自问 avahi 的结果）——以后这类问题一次导出即可定位。**注：本版的「换 D-Bus 路径」经实机验证不够用，已被 v0.6.3 的「容器内自建 dbus+avahi」取代；v0.6.3 起不再需要这个候选路径探测。** |
| v0.6.1 | 修复「环境构建成功但投屏黑屏」：**`ximagesink` 其实在独立包 `gstreamer1.0-x` 里，不在 `gstreamer1.0-plugins-base`**，旧 `APT_PACKAGES` 漏装它 → UxPlay 报 `no element "ximagesink"` 后渲染器崩溃。现补进安装清单，并同步修正「缺 sink 自动修复」分支与全部报错/README 救援命令（统一改成 `apt-get install -y gstreamer1.0-x …`）；`launcher.py` 致命检测新增 `no element "ximagesink"` 字样，避免无意义的无限重启。`is_ready()` 现在也校验渲染后端，且「安装/重建运行环境」在 **uxplay 版本已正确、只缺 sink** 时走**轻量修复**（apt 补 `gstreamer1.0-x` + 清缓存，几秒）而不再重下载编译 uxplay——旧容器点一下即可自愈。另：**桌面模式不再出现悬浮控制条**，`_will_takeover()` 只在游戏模式返回 True——桌面下既不隐藏主窗口也不显示置顶控制条（uxplay 窗口盖在上面显示画面，用任务栏 / `Alt+Tab` 切回即可），仅游戏模式（gamescope 无任务栏）保留该条作为返回入口 |
...
- **AppImage 双击无反应 / 启动报错**：SteamOS 镜像**自带 libfuse2**，AppImage 通常能直接跑，无需装任何系统包。
  ...
  `./AirPlayDeck-0.6.6-x86_64.AppImage`（或你实际的版本号文件名）看完整 traceback。若确实报 FUSE 相关错误（极少见），在桌面模式执行
...
- **有画面没声音（桌面 / 游戏模式都静音）**：容器内默认音频输出（ALSA/Pulse/JACK）在 SteamOS 上都不可用。
  **v0.6.4 起** App 会自动在容器里找宿主 PipeWire 的 PulseAudio 兼容 socket（首选 `/run/host/run/user/1000/pulse/native`），
  实测通过就把 `PULSE_SERVER=unix:<socket>`（必要时带 `PULSE_COOKIE`）透传给 uxplay，通常无需你动手。
  自检：日志里应出现 `音频已接入宿主机 PulseAudio（…）` 字样；若看到 `容器内找不到宿主机的 PulseAudio/PipeWire socket`，
  在 Deck 桌面终端跑这条看实况（列出容器可见的 pulse socket / cookie / pulsesink 插件 / 声卡）：
  ```bash
  podman exec uxplay-env sh -c 'ls -l /run/host/run/user/*/pulse/native /run/user/*/pulse/native 2>&1; ls -l /run/host/run/user/*/pulse/cookie 2>&1; gst-inspect-1.0 pulsesink >/dev/null 2>&1 && echo "pulsesink OK" || echo "pulsesink MISSING"; ls /dev/snd 2>&1'
  ```
  若报 `pulsesink MISSING`，补包并清缓存（免密码）：
  ```bash
  podman exec -u 0 uxplay-env bash -lc "apt-get update && apt-get install -y gstreamer1.0-pulseaudio && rm -rf /home/*/.cache/gstreamer-1.0 /root/.cache/gstreamer-1.0"
  ```
- **游戏模式搜得到但一投就退（`Failed to initialize GStreamer video renderer` / `Authorization required, but no authorization protocol specified`）**：
  实质是**容器里的 X 客户端拿不到对的那份 X cookie**（`/tmp/.X11-unix` 下 `X0`/`X1` 都在，但连上去被拒）。
  **v0.6.4 曾以为「游戏模式真正可用的是 `:0`」——这是错的**：实机日志显示 `:0` 和 `:1` 的所有尝试都失败，
  真因不在显示号，而在 cookie 候选被两个**根本不是 cookie** 的文件占满了。游戏模式 `/run/user/<uid>` 下有一堆
  `gamescope*` 文件，按 glob 排出来的头两个正好是 `gamescope-environment` / `gamescope-stats`（文本/统计文件），
  而每个显示号只实测 5 个组合 → 名额被它们占满，真正的 `gamescope.XXXXXX`（Xwayland 的 `-auth` 文件）**一次都没被试过**。
  **v0.6.5 起**改为读权威来源：扫宿主机 `/proc/*/cmdline`，把 Xorg/Xwayland 命令行里 `-auth <文件>` 按显示号取出来
  （`find_x_server_auth_map()`），该文件排在候选最前先试；`environment`/`stats` 这类明显不是 cookie 的文件沉到最后，
  每个显示号的实测名额也放宽到 8。日志里会先打印一行 `X 服务器 -auth 映射：:1 → /run/user/1000/gamescope.XXXXXX`。
  另外所有容器命令都带 `-e LD_PRELOAD= -e LD_LIBRARY_PATH=`，避免 Steam 的 32 位 `gameoverlayrenderer.so` 污染容器进程
  （`ld.so: wrong ELF class: ELFCLASS32`）。**游戏模式下不再有悬浮控制条**：投屏就是直接全屏出画面，结束后自动回到 App 主界面。
  若仍不行，把导出日志发我——现在里面会同时给出「`-auth` 映射」和每个候选的实测结果，一眼能看出卡在哪。
- **画面卡顿 / 不够流畅**：先看日志有没有这两句——
  `*** WARNING: raop_rtp resend failed: 22`（客户端反复要求重传）和
  `*** ERROR: N seconds since last client feedback request`（客户端反馈超时）。
  这两句一起出现，说明**投屏数据在 Wi-Fi 上丢了包**，不是渲染管线的问题（真的渲染出问题会直接黑屏 / 报
  `Failed to initialize GStreamer video renderer`，而不是「卡一下」）。按性价比依次试：
  1. **手机/平板与 Deck 都连 5GHz**，尽量靠近路由器；2.4GHz 拥挤时必卡。
  2. 路由器关掉「节能 / 省电 / Green AP」，手机端关掉「低电量模式」（这两者都会让 Wi-Fi 周期性进省电，表现为**规律性**卡顿）。
  3. **视频后端保持 `ximagesink`**（默认就是）。若你改成了 `xvimagesink`：它依赖 Xv 扩展，而 Xwayland / gamescope 下通常
     没有硬件加速实现，画面会发涩、偶尔停顿。v0.6.5 起选了 `xvimagesink` 会在启动日志里直接提示改回来。
  4. **帧率**：`60` 更顺滑但带宽翻倍，网络不稳时会**更**卡；网络好时再上 60，不稳就回 30。
  5. 分辨率保持 `auto`（向客户端请求 1280x800，Deck 原生；调大只会加重带宽）。
  Wi-Fi 稳下来后，`resend failed` 那串刷屏会消失，卡顿也就跟着没了。

- **日志刷 `no element "ximagesink"` 或 `Assertion 'renderer->sink' failed` / 退出码 134，始终搜得到连得上却没画面**：
  `ximagesink` 由**独立包 `gstreamer1.0-x`** 提供（**不在** `gstreamer1.0-plugins-base` 里！），
  旧版安装脚本漏装该包，加上 apt 锁中断可能把插件装坏、GStreamer **注册表缓存**又留着「插件不可用」的旧记录。
  在 Deck 桌面终端跑一次救援命令即可，无需重装 AppImage：
  ```bash
  ...
  ```
  看到 `ximagesink` 插件信息即修复成功，重开 App 点「开始接收」即可。
  **v0.6.1 起**安装清单已包含 `gstreamer1.0-x`，修复分支与报错提示也都指向该包，新装的容器不会再缺；v0.5.3 起 App 还会在安装/重建时自动清掉过期注册表缓存。
  **不想敲命令？** v0.6.1 里直接去「检查与日志」点「**安装 / 重建运行环境**」即可：App 会先查渲染后端，
  发现 uxplay 版本没问题、只缺 `ximagesink` 时走**轻量修复**（只补包 + 清缓存，几秒完成，不会重新编译 uxplay）。
- **日志刷 `*** ERROR: No DNS-SD Server found (DNSServiceRegister call returned kDNSServiceErr_Unknown)`，uxplay 存活 1 秒就退出并反复重启，手机/电脑搜不到设备**：
  这是**容器内找不到 mDNS 服务（avahi）**的典型症状。uxplay 用的是 `libavahi-compat-libdnssd`，它靠**系统 D-Bus** 找 avahi-daemon，
  而不是 `/run/avahi-daemon/socket`；而 rootless 容器里宿主的 D-Bus 通常用不了（容器内 uid 映射与 D-Bus EXTERNAL 鉴权打架，
  表现就是 `dbus-send` 报 `Did not receive a reply`），所以 v0.6.2「把 socket 换成 `/run/host/...`」的做法治不好，v0.6.3 改为在容器内自建一套。
  **v0.6.3 起 App 会自动在容器内自建一套 dbus + avahi**，正常情况下你什么都不用做——日志会写
  `启动容器内私有 system D-Bus…` / `容器内 avahi 就绪（mDNS 将直接广播到局域网）`。
  > ⚠️ **v0.6.3 / v0.6.4 的日志里那条「容器内仍无法使用 mDNS（avahi）」是误报，是探测脚本查错了总线名**：
  > avahi 注册的**总线名**是 `org.freedesktop.Avahi`，而 `org.freedesktop.Avahi.Server` 是它的**接口名**，
  > 用后者做 `--dest=` 永远得到 `ServiceUnknown`。也就是说那时 mDNS 其实一直是好的（设备照样连得上、能投屏），
  > 只是每次都被误判 + 白跑一遍自建流程。**v0.6.5 起**改为查 `ListNames` 里有没有 `org.freedesktop.Avahi`，
  > 不再误报，也省掉每次启动那几秒无用功。
  若确实搜不到设备，先跑这条看实况（**注意：要查总线名 `org.freedesktop.Avahi`，别查 `.Server`**）：
  ```bash
  podman exec uxplay-env sh -c 'ls -l /run/dbus /run/avahi-daemon 2>&1; which dbus-send dbus-daemon avahi-daemon; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/dbus/system_bus_socket dbus-send --system --print-reply --dest=org.freedesktop.DBus / org.freedesktop.DBus.ListNames | grep -i avahi || echo "总线上没有 Avahi"'
  ```
  看到 `string "org.freedesktop.Avahi"` 就说明容器内 mDNS 是好的。
  也可以手工在容器内起一套（与 App 做的是同一件事，免密码）：
  ```bash
  podman exec -u 0 uxplay-env sh -c 'apt-get install -y dbus avahi-daemon; mkdir -p /run/dbus /run/avahi-daemon; dbus-uuidgen --ensure; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/dbus/system_bus_socket dbus-daemon --system --fork --nopidfile; avahi-daemon --no-drop-root --no-chroot --no-rlimits --daemonize; sleep 2; DBUS_SYSTEM_BUS_ADDRESS=unix:path=/run/dbus/system_bus_socket dbus-send --system --print-reply --dest=org.freedesktop.DBus / org.freedesktop.DBus.ListNames | grep -i avahi'
  ```
  宿主侧可再确认 avahi 在跑、且 `avahi-daemon.conf` 里没有 `disable-publishing=yes`：
  `systemctl status avahi-daemon`、`avahi-browse -a -t`。
