...
if ! pip_install_target "AppDir/usr/lib/python$PYV/dist-packages" "PyQt6>=6.4"; then
  echo "[build] pip 安装 PyQt6 失败，请检查网络 / PyPI 可达性后重试。"
  echo "        若在容器内且网络慢，可换国内镜像后重试："
  echo "        python3 -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple --target=AppDir/usr/lib/python$PYV/dist-packages PyQt6"
  exit 1
fi
