...
def detect_system_lang() -> str:
    """按系统区域推断受支持的语言；不在支持列表（如 fr/de/ko）则回退英语。

    规则：zh + 地区 tw/hk/mo → 繁中，其余（cn/sg/无地区）→ 简中；
    ja → 日；en → 英；其它 → 英（按需求，不支持的系统语言默认英语）。
    """
    import os
...
S: Dict[str, Dict[str, str]] = {
    ...
    "dm_hint": {"zh_CN": "全屏=投屏后自动占满屏幕（默认）；窗口=保留可拖动窗口；自动=游戏全屏/桌面窗口。",
                "zh_TW": "全螢幕=投影後自動佔滿畫面（預設）；視窗=保留可拖動視窗；自動=遊戲全螢幕/桌面視窗。",
                "en": "Fullscreen: auto-fill the screen after mirroring (default). Window: keep a draggable window. Auto: fullscreen in game mode, window on desktop.",
                "ja": "全画面＝ミラーリング後に画面を埋め尽くす（既定）。ウィンドウ＝ドラッグ可能な窓を維持。自動＝ゲームモードは全画面／デスクトップは窓。"},
    ...
    "btn_uninstall": {"zh_CN": "一键清爽卸载", "zh_TW": "一鍵清爽解除安裝",
                      "en": "Clean uninstall", "ja": "クリーンアンインストール"},
    ...
    "uninstall_title": {"zh_CN": "一键清爽卸载", "zh_TW": "一鍵清爽解除安裝",
                        "en": "Clean uninstall", "ja": "クリーンアンインストール"},
    ...
    "uninstall_done": {"zh_CN": "已清爽卸载：{detail}", "zh_TW": "已清爽解除安裝：{detail}",
                       "en": "Cleaned up: {detail}", "ja": "クリーンアップ完了：{detail}"},
    ...
    "about_body": {
        ...
        "ja": (
            ...
            "全画面モード（ゲームモード含む）では上部に操作バーが表示されます。"
        ),
    },
