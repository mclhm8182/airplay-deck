...
class MainWindow(QMainWindow):
    ...
    def _will_fullscreen(self) -> bool:
        """本次接收是否以「接管屏幕」方式呈现（隐藏主窗口 + 悬浮控制条）。

        游戏模式必然接管（gamescope 只呈现全屏 X11 窗口）；
        桌面模式下「自动 / 全屏」接管屏幕（uxplay 窗口铺满 1280×800、显示全画面，
        但保留正常窗口而非无边框全屏），「窗口」则保留主窗口与可拖动小窗口，便于多任务。
        """
        if session.is_gamemode():
            return True
        return (self.settings.get("display_mode") or "auto") in ("fullscreen", "auto")
    ...
    def _check_env(self):
        """一键检查运行环境：uxplay 可用性 + 容器就绪状态，合并报告。"""
        ...
        if use_dbox:
            if cman.is_ready(container):
                self._log("info", f"容器 {container} 已就绪")
            else:
