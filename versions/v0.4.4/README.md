...
**目标形态：一个自包含的 `AirPlayDeck-<版本号>-x86_64.AppImage` 单文件**（如 `AirPlayDeck-0.4.4-x86_64.AppImage`），放到 Deck 的 `~/Applications`，
...
# 3) 产物 AirPlayDeck-<版本号>-x86_64.AppImage（当前 0.4.4）已生成在 ~/airplay-deck/
exit                         # 退出容器
```
...
1. 把构建产物 `AirPlayDeck-<版本号>-x86_64.AppImage`（当前 `AirPlayDeck-0.4.4-x86_64.AppImage`，文件名带版本号）放到 Deck 的 `~/Applications/`（如不存在就建一个；嫌版本号碍事可重命名为 `AirPlayDeck-x86_64.AppImage`）。
2. 终端执行：`chmod +x ~/Applications/AirPlayDeck-0.4.4-x86_64.AppImage`
...
在「检查与日志」页还能随时**一键安装 / 重建**运行环境，或**一键清爽卸载**（删容器 + 镜像 + 应用数据）。
...
| 显示模式 | auto | auto=游戏模式无边框全屏、桌面模式把 uxplay 窗口拉到铺满 1280×800 屏幕（正常窗口、非无边框全屏，靠 -s 1280x800 保证全画面不裁切，便于多任务）；fullscreen=任意模式都无边框全屏（游戏模式首选）；window=保留可拖动小窗口（横屏内容可能超出屏幕） |
...
- **AppImage 双击无反应 / 启动报错**：SteamOS 镜像**自带 libfuse2**，AppImage 通常能直接跑，无需装任何系统包。
  ...
  `./AirPlayDeck-0.4.4-x86_64.AppImage`（或你实际的版本号文件名）看完整 traceback。若确实报 FUSE 相关错误（极少见），在桌面模式执行
...
- **找不到 uxplay**：App 默认已开启「通过容器运行 uxplay」，首次打开会自动引导安装 `uxplay-env` 容器；若已跳过，去「检查与日志」点「安装 / 重建运行环境」。
