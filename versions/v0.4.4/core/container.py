...
def create_and_install(container: str, log_fn: LogFn) -> Tuple[bool, str]:
    ...
    # apt 锁（E: Could not get lock /var/lib/apt/lists/lock）。所以安装脚本先等初始化释放锁、
    # 再整体重试，避免一上来就因锁竞争而失败（9/9 复现的日志正是这个）。
    pkgs = " ".join(APT_PACKAGES)
    install_sh = (
        "#!/bin/bash\n"
        "# 等 distrobox 初始化释放 apt 锁；pgrep 缺失则跳过等待，交给下方重试\n"
        "for _i in $(seq 1 180); do\n"
        "  command -v pgrep >/dev/null 2>&1 || break\n"
        "  if ! pgrep -x apt-get >/dev/null 2>&1 && ! pgrep -x dpkg >/dev/null 2>&1; then break; fi\n"
        "  sleep 1\n"
        "done\n"
